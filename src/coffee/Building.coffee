class Building extends THREE.Mesh
	...
	update:(height,@color)->
		...
		TweenMax.to(@, .5, {r:( @color >> 16 & 255 ) / 255,g:( @color >> 8 & 255 ) / 255,b: ( @color & 255 ) / 255})
		TweenMax.to(@geometry.vertices[0], 1, {y : height, onUpdate:@needUpdateRequest})
		TweenMax.to(@geometry.vertices[1], 1, {y : height})
		TweenMax.to(@geometry.vertices[4], 1, {y : height})
		TweenMax.to(@geometry.vertices[5], 1, {y : height})
		return
